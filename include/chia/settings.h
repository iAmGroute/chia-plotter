/*
 * settings.h
 *
 *  Created on: Jun 7, 2021
 *      Author: mad
 */

#ifndef INCLUDE_CHIA_SETTINGS_H_
#define INCLUDE_CHIA_SETTINGS_H_

#include <cstdint>
#include <cstddef>

// SIZE is in bytes

constexpr size_t  G_READ_CHUNK_SIZE = 2 * 1024 * 1024; // Bytes of table entries to read at once (per read thread)
constexpr size_t G_WRITE_CHUNK_SIZE =      256 * 1024; // Bytes of table entries to buffer before write (per bucket)

//

constexpr int G_LOG_NUM_BUCKETS         =  7; // log2 of number of buckets in DiskSort

constexpr int G_TOTAL_THREADS           = 24;


constexpr int G_P1T1_CALC_THREADS       = 24; // tot

constexpr int G_P1T1_P1T1_WRITE_THREADS = 12; // tot  / 2


constexpr int G_P1_P1_SORT_THREADS      = 12; // tot  / 2
constexpr int G_P1_P1_READ_THREADS      = 12; // sort (og sort/2)

constexpr int G_P1_MATCH_THREADS        = 24; // tot
constexpr int G_P1_EVAL_THREADS         = 24; // tot

constexpr int G_P1_P1_WRITE_THREADS     = 12; // tot  / 2


constexpr int G_P2_P1F_READ_THREADS     =  2; // 2 (og tot/4)
constexpr int G_P2_P1F_READ_SIZE        = 4 * 1024 * 1024;

constexpr int G_P2_MARK_THREADS         = 24; // tot
constexpr int G_P2_REMAP_THREADS        = 24; // tot

constexpr int G_P2_P2_WRITE_THREADS     = 12; // tot  / 2


constexpr int G_P3S1_P1T1F_READ_THREADS =  2; // 2 (og tot/4)
constexpr int G_P3S1_P1T1F_READ_SIZE    = 4 * 1024 * 1024;

constexpr int G_P3S1_P2T7F_READ_THREADS =  2; // 2 (og tot/4)
constexpr int G_P3S1_P2T7F_READ_SIZE    = 4 * 1024 * 1024;

constexpr int G_P3S1_P3S1_WRITE_THREADS = 12; // tot  / 2

constexpr int G_P3S1_P2_SORT_THREADS    = 24; // tot (og tot/2 except t1f/t7f)
constexpr int G_P3S1_P2_READ_THREADS    = 12; // sort / 2

constexpr int G_P3S1_FILTER_THREADS     =  6; // tot  / 4
constexpr int G_P3S1_MERGE_THREADS      =  6; // tot  / 4


constexpr int G_P3S2_P3S1_SORT_THREADS  = 24; // tot
constexpr int G_P3S2_P3S1_READ_THREADS  = 12; // sort / 2

constexpr int G_P3S2_P3S2_WRITE_THREADS = 12; // tot  / 2
constexpr int G_P3S2_F_PARK_THREADS     = 12; // tot  / 2


constexpr int G_P4_P3S2_SORT_THREADS    = 24; // tot
constexpr int G_P4_P3S2_READ_THREADS    = 12; // sort / 2

constexpr int G_P4_F_PARK_THREADS       = 12; // tot  / 2
constexpr int G_P4_F_P7_THREADS         = 12; // tot  / 2


#endif /* INCLUDE_CHIA_SETTINGS_H_ */
